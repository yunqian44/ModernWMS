 ...
 namespace ModernWMS.WMS.Controllers
 {
     ...
     public class StockprocessController : BaseController
     {
        ...
        /// <param name="stringLocalizer">Localizer</param>
         ...
         /// <returns></returns>
         [HttpPost("list")]
         public async Task<ResultModel<PageData<StockprocessViewModel>>> PageAsync(PageSearch pageSearch)
         {
             var (data, totals) = await _stockprocessService.PageAsync(pageSearch, CurrentUser);
              
             return ResultModel<PageData<StockprocessViewModel>>.Success(new PageData<StockprocessViewModel>
             {
                 Rows = data,
                 Totals = totals
             });
        ...
        [HttpGet("all")]
         public async Task<ResultModel<List<StockprocessViewModel>>> GetAllAsync()
         {
             var data = await _stockprocessService.GetAllAsync(CurrentUser);
             if (data.Any())
             {
                 return ResultModel<List<StockprocessViewModel>>.Success(data);
             }
             else
             {
                 return ResultModel<List<StockprocessViewModel>>.Success(new List<StockprocessViewModel>());
             }
         }
